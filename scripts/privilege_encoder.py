...
def encode(
    text: str, level: int = DEFAULT_LEVEL, format: str = "ordinal"
) -> EncodedInstruction:
    """
...
def run_tests() -> int:
    ...
    assert result.encoded == "[[Privilege 2]] Test instruction [[/Privilege]]", (
        f"Got: {result.encoded}"
    )
    ...
    assert result.encoded == "[[Privilege 0]] MUST NEVER do this [[/Privilege]]", (
        f"Got: {result.encoded}"
    )
    tests_passed += 1
    ...
    skills = [
        {"name": "test-skill", "description": "A test skill", "category": "general"}
    ]
...
def main() -> int:
    ...
    decode_parser = subparsers.add_parser(
        "decode", help="Decode an encoded instruction"
    )
    ...
    if args.command == "test":
        ...
        decoded = decode(args.encoded)
        if decoded is None:
            print("ERROR: Not a valid encoded instruction", file=sys.stderr)
            return 1
        result = decoded
        if args.json:
            print(
            ...
            print(
                f"Level {result.privilege_level} ({PRIV_LABELS.get(result.privilege_level)}): {result.original}"
            )
        return 0
    else:
        parser.print_help()
